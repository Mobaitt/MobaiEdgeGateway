...
🌐 **在线演示**: https://edgegateway.mobai.site
...
脚本会先从 `origin/master` 拉取最新代码，再依次停止并移除旧容器、删除旧镜像、构建新镜像并启动服务；数据库和输出文件使用 Docker 命名卷持久化。脚本默认启用演示模式。部署目录需要是已配置远程仓库和访问凭据的 Git 工作区。
...
- MQTT 使用 MQTTnet 实际连接和发布，支持 `mqtt://host:1883`、`mqtts://host:8883`、用户名/密码、Client ID、主题及 QoS 0/1/2。TLS 使用系统证书校验。
- 连接或发布失败会报告失败；连接断开后在下一轮发送时尝试重连。当前不提供持久化离线消息队列。
- 通道编辑会释放旧策略并加载新配置；停用、删除及 HTTP 服务端路径变更会清理旧连接和端点缓存。
- 控制写入完成后必须成功回读且质量为 `Good` 才返回成功。回读失败会提示写入已完成但值未确认，并更新异常质量；应先检查设备实际状态，避免直接重复写入。
- Simulator 对写入值按点位类型校验，并在进程内跨连接保存；进程重启后清空。未写入的点仍按原有规则生成随机值。
### 🧠 数据处理

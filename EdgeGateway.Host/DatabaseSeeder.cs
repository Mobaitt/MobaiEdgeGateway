    ...
    public async Task InitializeTestDataAsync()
    {
        ...
        try
        {
            ...
            var virtualPoints = new List<VirtualDataPoint>
            {
                ...
                // 产线设备虚拟点
                new() { DeviceId = deviceProd.Id, Name = "平均温度", Tag = "SIM_PROD_LINE.Virtual.AvgTemp", Description = "平均工作温度", Expression = "Avg(SIM_PROD_LINE.SpindleTemp, SIM_PROD_LINE.Temperature)", CalculationType = CalculationType.Average, DataType = DataValueType.Float, Unit = "℃", DependencyTags = "[\"SIM_PROD_LINE.SpindleTemp\"]" },
